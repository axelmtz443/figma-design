...
function PlatformLabel({ label, color, isGradient }: { label: string; color: string; isGradient?: boolean }) {
  return (
    <div className="px-2 sm:px-3 py-1 sm:py-1.5 flex items-center gap-1 sm:gap-1.5 border-b border-white/5">
      <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full flex-shrink-0" style={{ background: color }} />
      <span
        className="text-[8px] sm:text-[9px] md:text-[10px] font-bold tracking-[0.12em] sm:tracking-[0.15em] uppercase"
        style={isGradient
  ...
  return (
    <div className="p-3 sm:p-4 flex justify-between items-start">
      <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
        <div className="relative w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-full border border-zinc-700/50 overflow-hidden bg-zinc-800 flex items-center justify-center flex-shrink-0 shadow-sm">
        ...
        <div className="min-w-0 flex flex-col justify-center">
          <div className="flex items-center space-x-0.5 sm:space-x-1">
            <span className="font-bold text-[13px] sm:text-[14px] md:text-[15px] text-[#e4e6eb] truncate leading-tight">{ad.pageName}</span>
            <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#0866ff] flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
          </div>
          <span className="text-[11px] sm:text-[12px] md:text-[13px] text-[#b0b3b8] font-normal leading-none text-left flex items-center">Publicidad <span className="mx-1 text-[9px] sm:text-[10px]">•</span></span>
        </div>
      </div>
      <button className="text-[#b0b3b8] hover:bg-[#3a3b3c] p-1.5 sm:p-2 rounded-full flex items-center justify-center transition flex-shrink-0">
...
function AdFooter({ ad, isWhatsApp }: { ad: MetaAd; isWhatsApp: boolean }) {
  return (
    <div className="bg-[#2f3031] px-3 sm:px-4 py-2.5 sm:py-3 flex justify-between items-center hover:bg-[#3a3b3c] transition group">
      <div className="flex-1 min-w-0 pr-3 sm:pr-4 text-left">
        <span className="block text-[9px] sm:text-[10px] md:text-[11px] text-[#b0b3b8] uppercase font-semibold tracking-wider truncate mb-0.5">{ad.ctaDomain}</span>
        <span className="block text-[14px] sm:text-[15px] md:text-[16px] font-bold text-[#e4e6eb] truncate leading-tight group-hover:underline">{ad.ctaTitle}</span>
        <span className="block text-[12px] sm:text-[13px] md:text-[14px] text-[#b0b3b8] truncate mt-0.5">{ad.ctaDesc}</span>
      </div>
      <div className="flex-shrink-0">
        {isWhatsApp ? (
          <button className="bg-[#25D366] hover:bg-[#1fa951] text-white font-bold text-[12px] sm:text-[13px] md:text-[14px] py-1.5 sm:py-2 px-3 sm:px-4 md:px-5 rounded-lg transition-colors flex items-center space-x-1.5 sm:space-x-2 shadow-sm">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12.012 2.002c-5.51 0-9.99 4.48-9.99 9.99 0 1.763.458 3.483 1.332 5.006L2.013 22l5.12-1.341a9.957 9.957 0 004.879 1.343c5.51 0 9.99-4.48 9.99-9.99a9.986 9.986 0 00-10-9.99zm5.06 14.1c-.22.61-1.28 1.13-1.78 1.18-.48.05-.98.07-3.12-.76-2.73-1.06-4.47-3.85-4.61-4.04-.13-.19-1.11-1.48-1.11-2.82 0-1.34.7-2 1-2.29.23-.23.61-.35.97-.35.12 0 .23 0 .33.01.29.01.44.02.63.48.24.58.82 2 .9 2.15.08.15.13.33.03.53-.1.2-.21.33-.37.52-.16.19-.34.42-.48.56-.16.16-.33.34-.14.67.19.32.85 1.41 1.83 2.28 1.26 1.13 2.32 1.48 2.65 1.65.3.15.48.13.66-.08.19-.23.82-.95 1.04-1.28.22-.33.44-.28.74-.17.3.11 1.91.9 2.24 1.07.33.16.55.24.63.38.08.14.08.82-.14 1.43z"/></svg>
            <span>{ad.ctaBtnText}</span>
          </button>
        ) : (
          <button className="bg-[#3a3b3c] hover:bg-[#4e4f50] text-[#e4e6eb] font-bold text-[12px] sm:text-[13px] md:text-[14px] py-1.5 sm:py-2 px-3 sm:px-4 md:px-5 rounded-lg transition-colors flex items-center shadow-sm">
      ...
      <div className="relative bg-black/50 group overflow-hidden border-y border-zinc-700/50 w-full flex items-center justify-center flex-1 min-h-0">
        {ad.videoUrl ? (
          <>
            ...
            {!isPlaying && (
              <button onClick={togglePlay} className="absolute w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-black/60 hover:bg-black/80 flex items-center justify-center border border-white/20 transition transform hover:scale-105 z-10 backdrop-blur-sm shadow-lg">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white ml-0.5 sm:ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
              </button>
            )}
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-2 sm:p-3 flex flex-col space-y-2 sm:space-y-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
              ...
              <div className="flex items-center justify-between px-1">
                <button onClick={togglePlay} className="text-white hover:text-[#0866ff] transition">
                  {isPlaying ? <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg> : <svg className="w-4 h-4 sm:w-5 sm:h-5 ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>}
                </button>
                <button onClick={toggleMute} className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-black/50 hover:bg-black/70 flex items-center justify-center text-white transition backdrop-blur-sm border border-white/10">
                  {isMuted ? <svg className="w-[14px] h-[14px] sm:w-[16px] sm:h-[16px] md:w-[18px] md:h-[18px]" fill="currentColor" viewBox="0 0 24 24"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM4 9v6h4l5 5V4L8 9H4zM19 12c0 2.97-1.75-5.51-4.25 6.64l1.42 1.42C19.34 18.33 21 15.35 21 12s-1.66-6.33-4.83-8.06l-1.42 1.42C17.25 6.49 19 9.03 19 12z"/></svg> : <svg className="w-[14px] h-[14px] sm:w-[16px] sm:h-[16px] md:w-[18px] md:h-[18px]" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L8 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>}
                </button>
              </div>
            </div>
          </>
        ) : (
          <div className="w-full h-[200px] sm:h-[220px] md:h-[240px] bg-[#18191a]" />
        )}
      </div>
...
function CarouselAd({ ad }: { ad: MetaAd }) {
  ...
  return (
    <div className="w-full bg-[#242526]/95 backdrop-blur-sm rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-zinc-700/50 overflow-hidden font-sans flex-shrink-0 h-full flex flex-col transition-all duration-300">
      ...
      <AdHeader ad={ad} />
      <ExpandableText text={ad.mainText} maxLength={300} />
      <div className="relative border-y border-zinc-700/50 bg-black/30 pt-2 sm:pt-3 pb-3 sm:pb-4 px-2 sm:px-3 overflow-hidden flex-1 min-h-0">
        <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${scrollIndex * (100 / visibleCards)}%)` }}>
          {cards.map((card, idx) => (
            <div key={idx} className="w-[220px] sm:w-[240px] md:w-[270px] flex-shrink-0 mr-2 sm:mr-3 bg-[#242526]/80 backdrop-blur-sm rounded-xl border border-zinc-700/50 overflow-hidden shadow-md flex flex-col group">
              ...
              <div className="p-3 sm:p-4 flex-grow flex flex-col justify-between bg-[#242526]/80 border-t border-zinc-700/50">
                <div className="min-w-0 space-y-0.5 sm:space-y-1 text-left">
                  <span className="block text-[8px] sm:text-[9px] md:text-[10px] text-[#b0b3b8] font-bold tracking-wider uppercase truncate">{card.domain}</span>
                  <span className="block text-[13px] sm:text-[14px] md:text-[15px] font-bold text-[#e4e6eb] truncate leading-snug group-hover:underline">{card.title}</span>
                </div>
                <div className="mt-3 sm:mt-4">
                  <button className="w-full bg-[#25D366] hover:bg-[#1fa951] text-white font-bold text-[12px] sm:text-[13px] md:text-[14px] py-1.5 sm:py-2 px-2 sm:px-3 rounded-lg flex items-center justify-center space-x-1.5 sm:space-x-2 transition-colors shadow-sm">
                    <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12.012 2.002c-5.51 0-9.99 4.48-9.99 9.99 0 1.763.458 3.483 1.332 5.006L2.013 22l5.12-1.341a9.957 9.957 0 004.879 1.343c5.51 0 9.99-4.48 9.99-9.99a9.986 9.986 0 00-10-9.99zm5.06 14.1c-.22.61-1.28 1.13-1.78 1.18-.48.05-.98.07-3.12-.76-2.73-1.06-4.47-3.85-4.61-4.04-.13-.19-1.11-1.48-1.11-2.82 0-1.34.7-2 1-2.29.23-.23.61-.35.97-.35.12 0 .23 0 .33.01.29.01.44.02.63.48.24.58.82 2 .9 2.15.08.15.13.33.03.53-.1.2-.21.33-.37.52-.16.19-.34.42-.48.56-.16.16-.33.34-.14.67.19.32.85 1.41 1.83 2.28 1.26 1.13 2.32 1.48 2.65 1.65.3.15.48.13.66-.08.19-.23.82-.95 1.04-1.28.22-.33.44-.28.74-.17.3.11 1.91.9 2.24 1.07.33.16.55.24.63.38.08.14.08.82-.14 1.43z"/></svg>
        ...
        {scrollIndex > 0 && (
          <button onClick={() => setScrollIndex(p => p - 1)} className="absolute left-2 sm:left-3 md:left-4 top-1/2 -translate-y-1/2 w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-full bg-[#242526] border border-zinc-700 shadow-[0_2px_8px_rgba(0,0,0,0.5)] flex items-center justify-center text-[#e4e6eb] hover:bg-[#3a3b3c] transition-colors z-10">
            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
          </button>
        )}
        {scrollIndex < maxIndex && (
          <button onClick={() => setScrollIndex(p => p + 1)} className="absolute right-2 sm:right-3 md:right-4 top-1/2 -translate-y-1/2 w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-full bg-[#242526] border border-zinc-700 shadow-[0_2px_8px_rgba(0,0,0,0.5)] flex items-center justify-center text-[#e4e6eb] hover:bg-[#3a3b3c] transition-colors z-10">
            <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
          </button>
        )}
      </div>
...
function InstagramCarouselAd({ ad }: { ad: MetaAd }) {
  ...
  return (
    <div className="w-full bg-[#000] rounded-xl shadow-2xl border border-zinc-800/60 overflow-hidden font-sans flex-shrink-0 h-full flex flex-col">
      ...
      <div className="px-2 sm:px-3 py-2 sm:py-2.5 flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-2.5">
          <div className="p-[2px] rounded-full bg-gradient-to-br from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] flex-shrink-0">
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full overflow-hidden border-[2px] border-black">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-0.5 sm:gap-1">
              <span className="text-white font-semibold text-[11px] sm:text-[12px] md:text-[13px] truncate max-w-[140px] sm:max-w-[160px] md:max-w-[180px]">{ad.pageName}</span>
              <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-[#0095f6] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm-1.177 14.28l-3.54-3.541 1.415-1.414 2.125 2.124 4.596-4.597 1.415 1.414-6.011 6.014z"/></svg>
            </div>
            <span className="text-zinc-400 text-[9px] sm:text-[10px] md:text-[11px] block leading-none">Publicidad</span>
          </div>
        </div>
      ...
      >
        ...
        {currentIndex > 0 && (
          <button onClick={() => goTo(currentIndex - 1)} className="absolute left-1.5 sm:left-2 top-1/2 -translate-y-1/2 w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-white/85 flex items-center justify-center shadow-md hover:bg-white transition z-10">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-black" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5"/></svg>
          </button>
        )}
        {currentIndex < cards.length - 1 && (
          <button onClick={() => goTo(currentIndex + 1)} className="absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-white/85 flex items-center justify-center shadow-md hover:bg-white transition z-10">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-black" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg>
          </button>
        )}
        <div className="absolute top-2 sm:top-3 right-2 sm:right-3 bg-black/55 text-white text-[9px] sm:text-[10px] md:text-[11px] font-semibold px-1.5 sm:px-2 py-0.5 rounded-full backdrop-blur-sm">{currentIndex + 1} / {cards.length}</div>
      </div>
      <div className="bg-[#0a0a0a] border-b border-zinc-800/60 px-2.5 sm:px-3 py-2 sm:py-2.5 flex items-center justify-between gap-2 sm:gap-3">
        <div className="min-w-0">
          <span className="text-[8px] sm:text-[9px] md:text-[10px] text-zinc-500 uppercase font-bold tracking-wide block">{cards[currentIndex].domain}</span>
          <span className="text-white text-[11px] sm:text-[12px] md:text-[13px] font-semibold truncate block">{cards[currentIndex].title}</span>
        </div>
        <button className="flex-shrink-0 bg-[#0095f6] hover:bg-[#007acc] text-white font-bold text-[10px] sm:text-[11px] md:text-[12px] px-3 sm:px-4 py-1 sm:py-1.5 rounded flex items-center gap-1 sm:gap-1.5 transition shadow whitespace-nowrap">
          <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M12.012 2.002c-5.51 0-9.99 4.48-9.99 9.99 0 1.763.458 3.483 1.332 5.006L2.013 22l5.12-1.341a9.957 9.957 0 004.879 1.343c5.51 0 9.99-4.48 9.99-9.99a9.986 9.986 0 00-10-9.99zm5.06 14.1c-.22.61-1.28 1.13-1.78 1.18-.48.05-.98.07-3.12-.76-2.73-1.06-4.47-3.85-4.61-4.04-.13-.19-1.11-1.48-1.11-2.82 0-1.34.7-2 1-2.29.23-.23.61-.35.97-.35.12 0 .23 0 .33.01.29.01.44.02.63.48.24.58.82 2 .9 2.15.08.15.13.33.03.53-.1.2-.21.33-.37.52-.16.19-.34.42-.48.56-.16.16-.33.34-.14.67.19.32.85 1.41 1.83 2.28 1.26 1.13 2.32 1.48 2.65 1.65.3.15.48.13.66-.08.19-.23.82-.95 1.04-1.28.22-.33.44-.28.74-.17.3.11 1.91.9 2.24 1.07.33.16.55.24.63.38.08.14.08.82-.14 1.43z"/></svg>
          {cards[currentIndex].ctaText}
        </button>
      </div>
      <div className="flex justify-center gap-1 sm:gap-1.5 pt-1.5 sm:pt-2 pb-0.5">
        {cards.map((_, idx) => (
          <button key={idx} onClick={() => goTo(idx)} className={`rounded-full transition-all duration-200 ${idx === currentIndex ? 'w-1.5 h-1.5 sm:w-2 sm:h-2 bg-[#0095f6]' : 'w-1 h-1 sm:w-1.5 sm:h-1.5 bg-zinc-600 hover:bg-zinc-400'}`} />
        ))}
      </div>
      <div className="px-2.5 sm:px-3 pt-1 pb-1 flex items-center justify-between">
        <div className="flex items-center gap-0.5 sm:gap-1">
          {['heart','comment','send'].map(icon => (
            <button key={icon} className="text-white hover:text-zinc-400 transition p-1 sm:p-1.5">
              ...
              {icon === 'comment' && <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z"/></svg>}
              {icon === 'send' && <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"/></svg>}
            </button>
          ))}
        </div>
        <button className="text-white hover:text-zinc-400 transition p-1 sm:p-1.5">
          <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z"/></svg>
        </button>
      </div>
      <div className="px-2.5 sm:px-3 pb-3 sm:pb-4 text-left">
        <p className="text-white text-[11px] sm:text-[12px] md:text-[13px] leading-snug whitespace-pre-wrap">
          <span className="font-semibold">{pageHandle} </span>
          {displayCaption}
          {isLong && !expanded && <button onClick={() => setExpanded(true)} className="text-zinc-400 text-[11px] sm:text-[12px] md:text-[13px] ml-0.5">más</button>}
        </p>
      </div>
    </div>
  );
...
function TikTokVideoAd({ ad }: { ad: MetaAd }) {
  ...
  return (
    <div className="w-full bg-[#161823] rounded-xl shadow-2xl border border-zinc-800/40 overflow-hidden font-sans flex-shrink-0 h-full flex flex-col">
      <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 flex items-center gap-1 sm:gap-1.5 border-b border-white/5">
        <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.28 6.28 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.67a8.18 8.18 0 004.79 1.52V6.72a4.85 4.85 0 01-1.02-.03z" style={{ fill: 'white' }}/></svg>
        <span className="text-[8px] sm:text-[9px] md:text-[10px] font-bold tracking-[0.12em] sm:tracking-[0.15em] uppercase text-white">TikTok Ads</span>
      </div>
      <div className="relative bg-black overflow-hidden flex-1 min-h-0" onClick={togglePlay}>
        ...
        <div className="absolute top-0 left-0 right-0 h-16 sm:h-20 bg-gradient-to-b from-black/60 to-transparent pointer-events-none" />
        <div className="absolute bottom-0 left-0 right-0 h-40 sm:h-44 md:h-52 bg-gradient-to-t from-black/95 to-transparent pointer-events-none" />
        {!isPlaying && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="w-11 h-11 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full bg-black/40 flex items-center justify-center backdrop-blur-sm border border-white/20">
              <svg className="w-5 h-5 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white ml-0.5 sm:ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
            </div>
          </div>
        )}
        <div className="absolute top-0 left-0 right-0 pt-2 sm:pt-3 pb-1.5 sm:pb-2 flex items-center justify-center z-20 pointer-events-none">
          <span className="text-white/50 text-[11px] sm:text-[12px] md:text-[13px] font-medium px-2 sm:px-3">Siguiendo</span>
          <span className="text-white text-[11px] sm:text-[12px] md:text-[13px] font-bold px-2 sm:px-3 border-b-2 border-white pb-0.5">Para ti</span>
          <span className="text-white/50 text-[11px] sm:text-[12px] md:text-[13px] font-medium px-2 sm:px-3">Amigos</span>
        </div>
        <button onClick={toggleMute} className="absolute top-10 sm:top-12 right-2 sm:right-3 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-black/40 flex items-center justify-center text-white backdrop-blur-sm border border-white/10 z-10">
          {isMuted ? <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM4 9v6h4l5 5V4L8 9H4zM19 12c0 2.97-1.75 5.51-4.25 6.64l1.42 1.42C19.34 18.33 21 15.35 21 12s-1.66-6.33-4.83-8.06l-1.42 1.42C17.25 6.49 19 9.03 19 12z"/></svg> : <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L8 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>}
        </button>
        <div className="absolute right-1.5 sm:right-2 md:right-2.5 bottom-28 sm:bottom-30 md:bottom-36 flex flex-col items-center gap-2.5 sm:gap-3 md:gap-4 z-20">
          <div className="relative mb-0.5 sm:mb-1">
            <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-full overflow-hidden border-2 border-white shadow-md">
              <ImageWithFallback src={ad.pageLogo} fallback={`https://ui-avatars.com/api/?name=${encodeURIComponent(ad.pageName)}&background=333&color=fff`} alt={ad.pageName} className="w-full h-full object-cover" />
            </div>
            <div className="absolute -bottom-1.5 sm:-bottom-2 left-1/2 -translate-x-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-[#ff0050] flex items-center justify-center border border-[#161823]">
              <svg className="w-1.5 h-1.5 sm:w-2 sm:h-2 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12 5v14m-7-7h14"/></svg>
            </div>
          </div>
          {[{ icon: 'heart', count: '4.8k' }, { icon: 'comment', count: '312' }, { icon: 'share', count: 'Reenviar' }].map(({ icon, count }) => (
            <div key={icon} className="flex flex-col items-center gap-0.5">
              <button className="text-white p-0.5 sm:p-1" onClick={e => e.stopPropagation()}>
                {icon === 'heart' && <svg className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"/></svg>}
                {icon === 'comment' && <svg className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z"/></svg>}
                {icon === 'share' && <svg className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 103.935 2.186 2.25 2.25 0 00-3.935-2.186zm0-12.814a2.25 2.25 0 103.933-2.185 2.25 2.25 0 00-3.933 2.185z"/></svg>}
              </button>
              <span className="text-white text-[8px] sm:text-[9px] md:text-[10px] font-semibold">{count}</span>
            </div>
          ))}
          <div className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full border-2 border-white/30 bg-zinc-800/60 flex items-center justify-center animate-[spin_4s_linear_infinite]">
            <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3v9.28a4 4 0 00-1-.28 4 4 0 100 8 4 4 0 004-4V7h4V3h-7z"/></svg>
          </div>
        </div>
        <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-10 sm:right-12 md:right-14 space-y-1.5 sm:space-y-2 z-20">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <span className="text-white font-bold text-[10px] sm:text-[11px] md:text-[13px]">@{ad.pageName.split(' ')[0].toLowerCase().replace(/[^a-z0-9]/g, '')}</span>
            <span className="bg-[#ff0050] text-white text-[7px] sm:text-[8px] md:text-[9px] font-bold px-1 sm:px-1.5 py-0.5 rounded">Patrocinado</span>
          </div>
          <p className="text-white text-[10px] sm:text-[11px] md:text-[12px] leading-snug opacity-90 line-clamp-2">{ad.mainText.split('\n')[0]}</p>
          <button className="w-full flex items-center justify-center gap-1.5 sm:gap-2 bg-white/15 backdrop-blur-sm border border-white/25 text-white font-bold text-[10px] sm:text-[11px] md:text-[12px] px-3 sm:px-4 py-1.5 sm:py-2 rounded-full hover:bg-white/25 transition">
            <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25"/></svg>
            {ad.ctaBtnText}
          </button>
          <div className="flex items-center gap-1.5 sm:gap-2 pt-0.5">
            <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white flex-shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3v9.28a4 4 0 00-1-.28 4 4 0 100 8 4 4 0 004-4V7h4V3h-7z"/></svg>
            <span className="text-white text-[8px] sm:text-[9px] md:text-[10px] opacity-75 truncate">Audio original · {ad.pageName.split(' ')[0]}</span>
          </div>
        </div>
    ...
    <div className="w-full bg-[#1b1f23] rounded-lg shadow-xl border border-[#283039] overflow-hidden font-sans flex-shrink-0 h-full flex flex-col">
      ...
      <div className="p-3 sm:p-4 flex items-start justify-between gap-2 sm:gap-3">
        <div className="flex items-start gap-2 sm:gap-3 flex-1 min-w-0">
          <div className="w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded overflow-hidden flex-shrink-0 border border-zinc-700/60">
          ...
          <div className="flex-1 min-w-0">
            <span className="text-white font-semibold text-[13px] sm:text-[14px] md:text-[15px] leading-tight block">{ad.pageName}</span>
            <span className="text-zinc-400 text-[10px] sm:text-[11px] md:text-[12px] block">Uniformes corporativos · 500+ seguidores</span>
            <div className="flex items-center gap-1 mt-0.5">
              <span className="text-zinc-500 text-[9px] sm:text-[10px] md:text-[11px]">Promocionado</span>
            </div>
          </div>
        </div>
        <button className="flex-shrink-0 border border-[#0a66c2] text-[#0a66c2] px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-[11px] sm:text-[12px] md:text-[13px] font-bold hover:bg-[#0a66c2]/10 transition flex items-center gap-1 whitespace-nowrap">
          <span className="text-base leading-none">+</span><span>Seguir</span>
        </button>
      </div>
      <div className="px-3 sm:px-4 pb-2.5 sm:pb-3 text-[12px] sm:text-[13px] md:text-[14px] text-zinc-200 text-left leading-relaxed whitespace-pre-wrap">
        {displayText}
      ...
      <div className="mx-2 sm:mx-3 mt-2 sm:mt-3 rounded border border-[#38434f] bg-[#283039] p-2.5 sm:p-3 flex items-center justify-between gap-2 sm:gap-3">
        <div className="min-w-0">
          <span className="text-white text-[12px] sm:text-[13px] md:text-[14px] font-semibold block truncate">{ad.ctaTitle}</span>
          <span className="text-zinc-400 text-[10px] sm:text-[11px] md:text-[12px] block">{ad.ctaDesc}</span>
        </div>
        <button className="flex-shrink-0 border border-[#0a66c2] text-[#0a66c2] px-3 sm:px-4 py-1 sm:py-1.5 rounded text-[11px] sm:text-[12px] md:text-[13px] font-bold hover:bg-[#0a66c2]/10 transition whitespace-nowrap">{ad.ctaBtnText}</button>
      </div>
      <div className="px-3 sm:px-4 pt-2.5 sm:pt-3 pb-1.5 sm:pb-2 flex items-center justify-between">
        <div className="flex items-center gap-1 sm:gap-1.5">
          <div className="flex -space-x-0.5">
            <span className="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-[#0a66c2] flex items-center justify-center text-[6px] sm:text-[7px] md:text-[8px] border border-[#1b1f23]">👍</span>
            <span className="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-[#df704d] flex items-center justify-center text-[6px] sm:text-[7px] md:text-[8px] border border-[#1b1f23]">❤️</span>
          </div>
          <span className="text-zinc-400 text-[10px] sm:text-[11px] md:text-[12px]">127</span>
        </div>
        <div className="flex items-center gap-1 sm:gap-2 text-zinc-400 text-[10px] sm:text-[11px] md:text-[12px]">
          <span>28 comentarios</span><span>·</span><span>12 reenvíos</span>
        </div>
      </div>
      <div className="mx-3 sm:mx-4 border-t border-[#38434f]" />
      <div className="px-1.5 sm:px-2 py-0.5 sm:py-1 flex items-center">
        {[
          { label: 'Recomendar', icon: <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5"/></svg> },
          { label: 'Comentar', icon: <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z"/></svg> },
          { label: 'Repostear', icon: <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99"/></svg> },
          { label: 'Enviar', icon: <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"/></svg> },
        ].map(({ label, icon }) => (
          <button key={label} className="flex-1 flex flex-col items-center gap-0.5 py-1.5 sm:py-2 rounded hover:bg-[#283039] transition text-zinc-400 hover:text-white group">
            <span className="group-hover:text-white transition">{icon}</span>
            <span className="text-[9px] sm:text-[10px] md:text-[11px] font-medium">{label}</span>
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <section id="redes-sociales" className="relative min-h-screen bg-transparent flex flex-col justify-between py-10 sm:py-12 md:py-16 lg:py-20 overflow-x-hidden border-t border-zinc-900/40">
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-12 flex flex-col items-start gap-3 sm:gap-4 mb-8 sm:mb-10 md:mb-12 lg:mb-16">
        <span className="text-[10px] sm:text-xs font-bold tracking-[0.2em] sm:tracking-[0.25em] uppercase text-[#0866ff]">Alcance masivo y segmentación precisa</span>
      ...
      <div className="w-full flex justify-center mt-8 sm:mt-10 lg:mt-12 mb-4 relative z-10">
        <button onClick={() => openPopup('Publicidad en Meta (Facebook & Instagram)')} className="inline-flex items-center justify-center px-6 sm:px-8 lg:px-10 py-3 sm:py-4 text-sm sm:text-base lg:text-lg font-bold text-white bg-gradient-to-r from-[#0866ff] to-[#00c6ff] rounded-full shadow-[0_0_20px_rgba(8,102,255,0.3)] hover:shadow-[0_0_30px_rgba(8,102,255,0.5)] hover:scale-105 transition-all duration-300">
          Cotizar Campaña
        </button>
      </div>
    </section>
