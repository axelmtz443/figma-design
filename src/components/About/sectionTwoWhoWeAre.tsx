...
const GlassCard = ({ title, description, Icon, color, progress, endStyle, LogoComponent, isMobile }: any) => {
  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 flex h-full ${isMobile ? 'flex-row items-center gap-3 p-3' : 'flex-col p-6 md:p-10'}`} style={{ opacity: contentOpacity, pointerEvents: progress > 0.9 ? 'auto' : 'none' }}>

        <div
          className={isMobile ? 'inline-flex w-fit flex-shrink-0 rounded-xl p-2 shadow-lg' : 'mb-4 md:mb-6 inline-flex w-fit rounded-2xl p-3 md:p-4 shadow-lg'}
          style={{
            backgroundColor: `rgba(${rgb}, 0.1)`,
            boxShadow: `0 0 20px rgba(${rgb}, 0.15)`
          }}
        >
          <Icon className={isMobile ? 'h-4 w-4' : 'h-6 w-6 md:h-8 md:w-8'} strokeWidth={1.5} style={{ color: color }} />
        </div>

        <div className={isMobile ? 'flex-1 min-w-0' : 'contents'}>
          <h3 className={isMobile ? 'text-[15px] font-bold tracking-wide font-montserrat text-white leading-tight' : 'mb-2 md:mb-4 text-2xl md:text-3xl font-bold tracking-wide font-montserrat text-white'} style={{ color: color }}>
            {title}
          </h3>

          <p className={isMobile ? 'text-white/80 leading-snug text-[10px] font-montserrat' : 'text-white/80 leading-relaxed text-[14px] md:text-[15px] font-montserrat mt-auto'}>
            {description}
          </p>
        </div>
      </div>
